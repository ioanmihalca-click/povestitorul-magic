<div class="py-8">
        ...
        <div x-data="{ open: @entangle('selectedGenre').defer === '{{ $genre }}' }" 
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-300"
                ...
                @if($selectedTheme === 'custom')
                    <input wire:model="customTheme" type="text" placeholder="Scrie-ți propria aventură aici" 
    ...
    @endforeach
                    ...
                    @error('selectedGenre') <span class="text-sm text-red-500">{{ $message }}</span> @enderror
                    @error('selectedTheme') <span class="text-sm text-red-500">{{ $message }}</span> @enderror
                    @error('customTheme') <span class="text-sm text-red-500">{{ $message }}</span> @enderror

                    <div class="text-center">
                        <button type="submit" class="px-8 py-3 text-xl font-bold text-white transition duration-300 transform rounded-full shadow-lg bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 hover:scale-105">
