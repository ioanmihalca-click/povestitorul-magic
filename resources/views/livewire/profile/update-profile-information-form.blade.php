...
<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900">
            {{ __('Informatii de profil') }}
        </h2>

        <p class="mt-1 text-sm text-gray-600">
            {{ __("Modifica informatiile de profil si adresa de email") }}
        </p>
    </header>
            ...
            @if (auth()->user() instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! auth()->user()->hasVerifiedEmail())
                <div>
                    <p class="mt-2 text-sm text-gray-800">
                        {{ __('Adresa ta de email nu este verificata') }}
                        ...
                        <button wire:click.prevent="sendVerification" class="text-sm text-gray-600 underline rounded-md hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            {{ __('Click aici pentru a re-trimite codul de verificare') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 text-sm font-medium text-green-600">
                            {{ __('Un nou link de verificare a fost trimis pe adresa ta de email') }}
                        </p>
                    @endif
                </div>
            @endif
